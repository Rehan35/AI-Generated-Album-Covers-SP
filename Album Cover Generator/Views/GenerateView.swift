...
struct GenerateView: View {
    ...
    @State private var generatedImageURL = ""
...
//                        submit()
                ...
                .navigationDestination(isPresented: $displayCompletedView) {
                    if let album = collection as? Album {
                        CompletedView(collection: collection, generatedImage: generatedImage, collectionName: album.name, isPlaylist: false, displayCompletedView: $displayCompletedView)
                            .environmentObject(spotify)
                    } else if let playlist = collection as? Playlist<PlaylistItemsReference> {
                        CompletedView(collection: collection, generatedImage: generatedImage, collectionName: playlist.name, isPlaylist: true, displayCompletedView: $displayCompletedView)
                            .environmentObject(spotify)
                    }
                }
    ...
    func openAIImageGeneration() {
        ...
        let apiKey = "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"
        
        var request = URLRequest(url: url)
        print(2)
        
        let data : [String: Any] = ["prompt": generation.positivePrompt, "n": 1, "size": "256x256"]
            ...
            let response = URLSession.shared.dataTask(with: request) { data, response, error in
                if let data = data {
                    do {
                        ...
                        print(json["data"] )
//                        self.generatedImageURL = json!["url"] as! String
//                        self.generatedImage = Image(systemName: "person.fill").data(url: URL(string: generatedImageURL)!)
